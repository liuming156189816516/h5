...
func InitEvent() error {
	...
	//消息回调
	err = natsRpc.HandlerDllNrpc("ReceiveMessagesEvent", api.ReceiveMessagesEventHandler)
	if err != nil {
		return err
	}
...
func main() {
	...
	logs.SetLogger(logs.AdapterFile, `{"filename":"./logs/message.log","separate":["debug","error"]}`)
